    ...
    <>
      ...
      <div style={{ transition: "width .8s" }} className='nav w-0 fixed right-0 h-[100vh] bg-gray-900 text-centers rounded-tl-xl rounded-bl-xl grid grid-rows-[20%_29%_29%_22%] select-none border-l-2 border-yellow-300'>
        <div style={{ transition: "opacity .5s" }} className='divv relative flex-col pl-6 pr-6 pt-3 z-50'>
          ...
          <Link href="/Login"><button className="login relative top-9 text-slate-100 bg-transparent rounded-md p-2 border border-slate-300 text-xs uppercase hover:text-black font-semibold overflow-hidden">Login / Sigup</button></Link>
        ...
        <div className='div3 relative boorder border-t-[0.1vw] border-gray-800 pt-3 pl-6 pr-6 z-50'>
