  ...
  return (
    <>
      <div data-scroll data-scroll-section data-scroll-speed="-.4" className='ParentDiv w-[100%] h-[100vh] bg-[#A9E5FF] relative overflow-hidden rounded-bl-[2vw] rounded-br-[2vw] z-50'>
        <div className="relative logoIMG w-[6vw] h-[4vw] left-3 top-3">
          <Image id='logo' className='absolute object-contain' src={logo} alt='logo' fill sizes='20vw, 20vw' quality={100} />
        </div>
          ...
          <div className='relative w-full h-full flex'>
            <div className="child1 w-auto h-full flex items-center">
              ...
              {[summer, coupon1, trends, sale1, Winter, coupon3, formals].map((elements, index) => (
                <motion.div key={index} initial={{ scale: 1 }} whileHover={{ scale: 1.05 }} transition={{ ease: "easeOut", duration: 0.4 }} className="relative elem1 min-w-[16vw] h-[22vw] p-1 bg-white flex mr-3 z-20 shadow-md shadow-slate-500 shadow-b-2 shadow-r-[6px] -shadow-spread-6 ">
                  <Image key={index} src={elements} alt='ads' className='absolute object-cover p-1' fill sizes='50vw, 50vw' quality={100} />
                </motion.div>
              )
              )}
            </div>
            <div className="child1 w-auto h-full flex items-center">
              <h1 className='text1 absolute z-10 -top-14 text-8xl font-bold'>VOUGE FASHION TREND CLASSY</h1>
              {[summer, coupon1, trends, sale1, Winter, coupon3, formals].map((elements, index) => (
                <motion.div key={index} initial={{ scale: 1 }} whileHover={{ scale: 1.05 }} transition={{ ease: "easeOut", duration: 0.4 }} className="relative elem1 border min-w-[16vw] h-[22vw] p-1 bg-white shadow-md shadow-slate-500 shadow-b-2 shadow-r-[6px] -shadow-spread-6 flex mr-3 z-20">
                  <Image key={index} src={elements} alt='ads' className='absolute object-cover p-1' fill sizes='50vw, 50vw' quality={100} />
                </motion.div>
              )
              )}
          ...
          <div className='relative w-full h-full flex '>
            <div className="child2 w-auto h-full flex items-center">
              {[coupon2, casual, sale2, ethnic, coupon4, party, sale3].map((elements, index) => (
                <motion.div key={index} initial={{ scale: 1 }} whileHover={{ scale: 1.03 }} transition={{ ease: "easeOut", duration: 0.4 }} className="relative elem border min-w-[16vw] h-[22vw] p-1 bg-white shadow-md shadow-slate-500 shadow-b-2 shadow-r-[6px] -shadow-spread-6 flex mr-3 z-20">
                  <Image key={index} src={elements} alt='ads' className='absolute object-cover p-1' fill sizes='50vw, 50vw' quality={100} />
                </motion.div>
              )
              )}
